...
- 全程**没有任何 cmd 窗口/灰框**:启动器 `launch.vbs` 以隐藏模式运行 bat,
  服务进程也是隐藏子进程。
- 万一连不上,窗口会显示引导页(自动重连),或查看应用数据目录
  (`userdata/`,打包版在 exe 旁边的 `userdata/` 或系统 `%APPDATA%\dsh-desktop`)下的
  `main-error.log`。
...
**Q:一直停在"正在启动本地服务"?**
先确认 3080 端口有没有被占用,或看应用数据目录(`userdata/`)下的
`main-error.log` 里的报错(源码运行时也在同目录看 `launch.log`),再点"立即重试"。
